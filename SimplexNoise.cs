...
namespace CrawfisSoftware.Noise
{
    ...
    /// Based loosely on C source code by Stefan Gustavson: https://github.com/stegu/perlin-noise
    /// </summary>
    public class SimplexNoise
    {
        private readonly byte[] _permutations;

        private static readonly byte[] OriginalPermutations = {
        ...
        public SimplexNoise()
        {
            _permutations = new byte[OriginalPermutations.Length];
            OriginalPermutations.CopyTo(_permutations, 0);
        }

        /// <summary>
        /// Constructor.
        /// </summary>
        /// <param name="random">A new random generator to use only if overriding the default set of constants from the Simplified Perlin Noise. May introduce low frequency aliasing.</param>
        public SimplexNoise(System.Random random)
        {
            _permutations = new byte[512];
            random.NextBytes(_permutations);
        }
        ...
        /// <param name="scale">A scale factor to use in the noise generation.</param>
        /// <returns>A 3D array of floats.</returns>
        ...
        {
            float sample = Sample(x * scale);
            return sample;
        }

        /// <summary>
        /// Sample a 2D Simplex Noise
        /// </summary>
        /// <param name="x">An integer coordinate</param>
        ...
        public float Sample(float x)
        {
            ...
            var t0 = 1.0f - x0 * x0;
            t0 *= t0;
            var n0 = t0 * t0 * Gradient(_permutations[i0 & 0xff], x0);

            var t1 = 1.0f - x1 * x1;
            t1 *= t1;
            var n1 = t1 * t1 * Gradient(_permutations[i1 & 0xff], x1);
        ...
        public float Sample(float x, float y)
        {
            ...
            var ii = ModulaPositive(i, 256);
            var jj = ModulaPositive(j, 256);
            ...
            {
                t0 *= t0;
                n0 = t0 * t0 * Gradient(_permutations[ii + _permutations[jj]], x0, y0);
            }
            ...
            {
                t1 *= t1;
                n1 = t1 * t1 * Gradient(_permutations[ii + i1 + _permutations[jj + j1]], x1, y1);
            }
            ...
            {
                t2 *= t2;
                n2 = t2 * t2 * Gradient(_permutations[ii + 1 + _permutations[jj + 1]], x2, y2);
            }

            // Add contributions from each corner to get the final noise value.
            // The result is scaled to return values in the interval [-1,1].
            return 40.0f * (n0 + n1 + n2) * 0.5f + 0.5f; // TODO: The scale factor is preliminary!
        }
        ...
        public float Sample(float x, float y, float z)
        {
            ...
            {
                t0 *= t0;
                n0 = t0 * t0 * Gradient(_permutations[ii + _permutations[jj + _permutations[kk]]], x0, y0, z0);
            }
            ...
            {
                t1 *= t1;
                n1 = t1 * t1 * Gradient(_permutations[ii + i1 + _permutations[jj + j1 + _permutations[kk + k1]]], x1, y1, z1);
            }
            ...
            {
                t2 *= t2;
                n2 = t2 * t2 * Gradient(_permutations[ii + i2 + _permutations[jj + j2 + _permutations[kk + k2]]], x2, y2, z2);
            }
            ...
            {
                t3 *= t3;
                n3 = t3 * t3 * Gradient(_permutations[ii + 1 + _permutations[jj + 1 + _permutations[kk + 1]]], x3, y3, z3);
            }
        ...
        private static int ModulaPositive(int x, int m)
        {
            // Assumes m is positive, but x can be negative.
            var a = x % m;
            return a < 0 ? a + m : a;
        }
        ...
        private static float Gradient(int hash, float x, float y, float z, float t)
        {
